<template>
  <form @submit.prevent="handleSubmit" class="meetup-form">
...
export default {
  ...
  watch: {
    meetup: {
      ...
      immediate: true,
      handler(newValue) {
        if (!isEqual(this.localMeetup, newValue)) {
          this.localMeetup = { ...newValue };
        }
      },
    },
  ...
  methods: {
    ...
    remove(keyItem) {
      return this.localMeetup.agenda.splice(
        this.localMeetup.agenda.findIndex((el) => el.id === keyItem),
        1,
      );
    },
  },
